...
const days = [
  // ---------------- TOKIO ----------------
  {
    ...
    stops: [
      ...
      { name: 'Sumida', kind: 'visit', durOverride: 20, note: 'Barrio a orillas del río, a los pies del Skytree.' },
  ...
  },
  {
    ...
    stops: [
      ...
      { name: 'Mirador del Gobierno Metropolitano de Tokio', kind: 'visit', durOverride: 40, note: 'Junto al Park Hyatt Tokyo, escenario de "Lost in Translation".' },
  ...
  },
  {
    ...
    stops: [
      ...
      { name: 'Itoya Ginza', kind: 'visit', durOverride: 35, mealAfter: 'lunch', note: 'La papelería más famosa del mundo — varias plantas de material de escritura y papel de diseño.' },
      { name: 'Avenida Chuo-dori', kind: 'visit', durOverride: 30, note: 'De 12:00 a 17:00 (domingo) está cortada al tráfico — pasead por el centro de la calzada.' },
      { name: 'Seiko House Ginza (Wako)', kind: 'visit', durOverride: 20 },
      { name: 'Ginza Place', kind: 'visit', durOverride: 15 },
      { name: 'Almacenes Mitsukoshi Ginza', kind: 'visit', durOverride: 40, note: 'La estatua Ginza Shusse Jizoson de la azotea da buena suerte; en el sótano (depachika) hay puestos de comida.' },
      { name: 'Ginza', kind: 'visit', durOverride: 45, note: 'Callejeando por las bocacalles de Ginza — boutiques, galerías y cafés.' },
      { name: 'Ginza Six', kind: 'visit', durOverride: 45, mealAfter: 'dinner', note: 'El centro comercial más grande de Ginza, con la librería Tsutaya.', transit: 'Metro Toei Asakusa Line Higashi-Ginza → Asakusa, ~15 min.', transitMin: 20 },
      { name: 'Hotel Tokyo', kind: 'info' },
    ],
  },
  {
    ...
    stops: [
      ...
      { name: 'Estación de Tokio (Marunouchi)', kind: 'visit', durOverride: 20, note: 'Fachada histórica de ladrillo de 1914. Desde aquí sale el Shinkansen a Kanazawa.', transit: 'Shinkansen Hokuriku "Kagayaki" Tokio → Kanazawa, ~2h30.', transitMin: 150 },
      ...
      { name: 'Nagamachi', kind: 'visit', durOverride: 45, note: 'Barrio de samuráis, calles empedradas y muros de barro.', mealAfter: 'lunch' },
      ...
      { name: 'Higashi Chaya', kind: 'visit', durOverride: 45, note: 'El barrio de geishas más grande y fotogénico de Kanazawa.', mealAfter: 'dinner' },
  ...
  // ---------------- KANAZAWA / TAKAYAMA ----------------
  {
    ...
    stops: [
      ...
      { name: 'Shirakawa', kind: 'visit', durOverride: 240, note: 'Con una mañana (~4h) es suficiente, no hace falta el día completo.', transit: 'Bus Nohi Shirakawa-go → Takayama, ~50 min.', transitMin: 50 },
  ...
  },
  {
    ...
    stops: [
      ...
      { name: 'Teramachi', kind: 'visit', durOverride: 30, note: 'Paseo por el barrio de templos de la ladera este.' },
      { name: 'Parque Shiroyama', kind: 'visit', durOverride: 25, note: 'Antiguo enclave del castillo de Takayama.' },
  ...
  // ---------------- KIOTO ----------------
  {
    ...
    stops: [
      ...
      { name: 'Kiyomizu-dera', kind: 'visit', durOverride: 60, note: 'Ir a primera hora, antes de los grupos.' },
      ...
      { name: 'Monumentos de piedra y encantos de Pontocho', kind: 'visit', durOverride: 45, note: 'Ir sobre las 22h: mucha menos gente, farolillos encendidos (la hora del reloj de aquí abajo asume que llegáis según el ritmo del día, no a las 22h en punto).', mealAfter: 'dinner', transit: 'Paseo de 20 min junto al río Kamo, o autobús directo Kawaramachi → Shijo Karasuma (~10 min).', transitMin: 15 },
      { name: 'Hotel Kyoto', kind: 'info' },
    ],
  },
  {
    ...
    stops: [
      ...
      { name: 'Adashino Nenbutsuji Temple', kind: 'visit', durOverride: 30, note: '3€ de entrada, mucho más tranquilo que el resto de Arashiyama.', mealAfter: 'dinner', transit: 'JR Sagano Line Saga-Arashiyama → Kioto Station (~15-20 min) + bus/metro al hotel (~10 min).', transitMin: 30 },
      { name: 'Hotel Kyoto', kind: 'info' },
    ],
  },
  {
    ...
    stops: [
      ...
      { name: 'Ginkaku-ji', kind: 'visit', durOverride: 45, note: 'El "Pabellón de Plata" — entrada norte del Camino del Filósofo.' },
  ...
  },
  {
    ...
    stops: [
      ...
      { name: 'Nara', kind: 'visit', durOverride: 0, note: 'Probar un mochi de yomogi.', transit: 'Paseo por Sanjō-dori (calle comercial) hasta el parque, ~15 min — o bus Nara Kotsu, ~10 min.', transitMin: 10 },
      { name: 'Parque de Nara', kind: 'visit', durOverride: 45, note: 'Ciervos mansos.' },
  ...
  // ---------------- OSAKA ----------------
  {
    ...
    stops: [
      ...
      { name: 'Universal Studios Japan', kind: 'visit', durOverride: 480, note: 'Entre semana y pase express para 4 atracciones. Normal 48€, express 101€. Planificar atracciones en la app. Comida dentro del parque.', transit: 'JR Yumesaki Line Universal City → Nishikujo (~4 min) + JR Osaka Loop Line a Osaka/Temmabashi (~20 min).', transitMin: 30 },
  ...
  },
  {
    ...
    stops: [
      ...
      { name: 'Okonomimura', kind: 'visit', durOverride: 45, note: 'Comer aquí: okonomiyaki.' },
      ...
      { name: 'Calle Comercial Miyajima Omotesando', kind: 'visit', durOverride: 30, note: '18h cierran casi todas las tiendas.', transit: 'Bus gratuito a la estación del ropeway + telesférico Miyajima, ~15 min en total.', transitMin: 20 },
  ...
  },
  {
    ...
    stops: [
      ...
      { name: 'Castillo Osaka', kind: 'visit', durOverride: 75, note: 'Tiene ascensor con vistas; el exterior es más interesante que el interior.', transit: 'Metro Osaka Loop/Tanimachi Line Osakajōkōen → Umeda, ~15-20 min en total.', transitMin: 20 },
      { name: 'Umeda Sky Building (osaka)', kind: 'visit', durOverride: 45, note: 'Ir para la puesta de sol.', transit: 'Metro Midosuji Line Umeda → Namba (~10 min) + andar/metro a Kuromon (~10 min).', transitMin: 20 },
      ...
      { name: 'Dōtonbori', kind: 'visit', durOverride: 60, note: 'Mejor de noche. Restaurantes: Houzenji Sanpei y Mizuno.' },
